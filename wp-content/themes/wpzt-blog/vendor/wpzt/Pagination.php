...
class Pagination {
  ...
  function __construct ($total, $pageSize = 10) {
	...
	$this->current_url=str_replace('?'.$_SERVER['QUERY_STRING'],'',$_SERVER['REQUEST_URI']);
    $this->setQueryField();
    $this->resize();
  }

  function convertUrlQuery ($url) {	//parse_str()
	  $res = parse_url($url);
  ...
  function array2Query ($arr) {//http_build_query()
	  $keys = [];
  ...
  function setQueryField ($arr = []) {
    ...
    $params = $this->convertUrlQuery($_SERVER['REQUEST_URI']);
    // 去除分页查询字符串
  ...
  private function sizesHTML () {
    //$action = $path = $_SERVER['SCRIPT_NAME']. '?' .$this->array2Query($this->querys). '&'. $this->queryPageField .'=1';
	$action = $path = $this->current_url. '?' .$this->array2Query($this->querys). '&'. $this->queryPageField .'=1';
