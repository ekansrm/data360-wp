<?php
	...
	$count=$wpdb->get_var("select count(*) from {$wpdb->prefix}wpzt_order where  status=0 and type>=1 and type<=4 {$countsearchstr}");//vip
	...
	$query="select o.sn sn,u.user_login user,o.time time,o.price price,o.type type from {$wpdb->prefix}wpzt_order o 
	 left join {$wpdb->prefix}users u on o.uid=u.id where o.status=0 and o.type>=1 and o.type<=4 {$searchstr} ORDER BY o.id DESC limit {$startnum},{$pagesize}";
