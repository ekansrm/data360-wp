<?php
	...
	if(!empty($username)||!empty($ordersn)){//搜索表单提交
	...
	global $wpdb;
	$count=$wpdb->get_var("select count(*) from {$wpdb->prefix}wpzt_order where status=1 and type=1 {$countsearchstr}");
	...
	o.pid=p.id left join {$wpdb->prefix}users u on o.uid=u.id where o.status=1 and o.type=0 {$searchstr} ORDER BY o.id DESC limit {$startnum},{$pagesize}";
...
<div class="panel panel-default">
	...
	?>
	  ...
	  ?>
		...
		<td><?php echo empty($v->user)?'无登录支付':$v->user;?></td>
